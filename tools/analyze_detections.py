...
def analyze(path: Path, duplicate_iou: float, large_area_ratio: float) -> int:
    ...
    temps = [float(item["max_temp_c"]) for item in observations if isinstance(item.get("max_temp_c"), (int, float))]
    ...
    if temps:
        print(f"Temp C: min={min(temps):.2f}, median={statistics.median(temps):.2f}, max={max(temps):.2f}, n={len(temps)}")
    if area_ratios:
        print(
            "Area ratio: "
